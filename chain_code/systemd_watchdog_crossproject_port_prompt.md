...
>
> ★本檔為 Edit11→Edit12 同步副本。Edit12_Krank56002 的 `PREFIX` = `edit12-`、`THIS` = `Edit12_Krank56002`。
> Edit12 多半同樣是 Edit6/Edit11 抄來的 `edit6-`/`edit11-` 殘留,照本流程改成 `edit12-` + 指向 Edit12。

## 名詞(每專案替換)
- `THIS` = 本專案目錄名(此處 `Edit12_Krank56002`);`PREFIX` = 本專案 systemd 前綴(此處 `edit12-`)。
...
- 參考實作:Edit11 commit(2026-06-21)— 13 自指檔 sweep + CLAUDE.md 保護式換 + 命令 `/edit6-monitor`→`/edit11-monitor`
