#!/bin/bash
# ============================================================================
# grab_poller.sh — Edit11_Krank5600 NCHC 復機自動偵測搶投遞 (Edit11-only)
# ----------------------------------------------------------------------------
# 由 edit11-grab.timer (systemd --user) 每 ~30s 觸發,session-independent
# (linger=yes → 即使無 Claude session、甚至 login node 重開機後也自動續跑)。
#
# 目標:NCHC 維護停機 (2026-06-27 09:00 ~ 06-28 14:00) 結束、叢集一開放,就在
#   ~30s 內自動 warm 搶投 Edit11 的 job(免人手動打 lbm-grab,搶 GPU slot)。
#
# 設計:thin wrapper around `lbm-grab edit11`(已 Codex 驗證的 relauncher),所有
# 「難」的邏輯都重用它:
...
#   - Edit11-only = `lbm-grab edit11` 的 only-filter,絕不碰 Edit12/Edit13(遵守跨專案隔離)。
...
#               systemctl --user enable --now edit11-grab.timer
# ============================================================================
set -uo pipefail
...
# --- 2. Edit11 job 是否已在佇列?(以 WorkDir 驗歸屬,跨專案安全,不靠 job 名) ---
...
if [ -n "$mine" ]; then
  if [ -f "$LAUNCHED" ]; then
    ...
    touch "$DISARM"
    systemctl --user disable --now edit11-grab.timer >/dev/null 2>&1 || true
    echo "[$(TS)] GRABBED ✅ Edit11 job 進佇列 ($mine) → 自動解除 poller(.grab_disarmed + disable timer)" >> "$LOG"
  fi
  # 無 .grab_launched = 我們沒投過,這是別人/停機前還活著的 job → 靜默待命,不解除、不動作。
  exit 0
fi

# --- 3. 無 Edit11 job → 嘗試搶(lbm-grab edit11;Pass-0 會在維護未結束時擋下)---
...
echo "[$(TS)] 偵測無 Edit11 job → lbm-grab edit11(Pass-0 會擋維護中;rc=2=維護未結束屬正常)..." >> "$LOG"
out=$(bash -c 'source "$HOME/.lbm_nchc_grab.sh" 2>/dev/null; lbm-grab edit11' 2>&1); rc=$?
printf '%s\n' "$out" | sed 's/^/    /' >> "$LOG"
echo "[$(TS)] lbm-grab edit11 rc=$rc" >> "$LOG"
