...
# 【2026-06-22 移植自 Edit11_Krank5600 的 plumbing 版(取代舊 porcelain git add/commit 版):
#   舊版無 git fetch、非ff 即放棄、且 git add 會碰主 index/dirty 工作樹。新版離線健壯。】
#
...
# Edit12 SPF≈1577630 steps/FTT(由 restart log 33117001/20.9916 精算; NDTBIN 註解記 1577102, 整數標籤無差)
ftt=$(python3 -c "print(f'{${step:-0}/1577630:.0f}')" 2>/dev/null || echo '?')
