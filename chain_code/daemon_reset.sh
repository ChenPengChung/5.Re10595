#!/bin/bash
# 乾淨 reset Edit12 的 dispatcher + hill_watcher 成「單一 systemd 管理實例」。
# 現行守護: systemd user services (edit12-dispatcher / edit12-watcher,
#           Restart=on-failure + enable-linger) — 已完全脫離 crontab race。
# 本腳本用於恢復: 殺光所有殘留/重複的 Edit12 daemon (含「相對路徑」啟動的孤兒),
#               再交回 systemd 單一管理。
...
systemctl --user stop edit12-dispatcher.service edit12-watcher.service 2>/dev/null
sleep 2
...
systemctl --user start edit12-dispatcher.service edit12-watcher.service 2>/dev/null
sleep 4
...
echo "dispatcher service=$(systemctl --user is-active edit12-dispatcher.service) MainPID=$(systemctl --user show -p MainPID --value edit12-dispatcher.service)  本專案實例(cwd)=$(cnt_by_cwd submit_dispatcher.sh)"
echo "watcher    service=$(systemctl --user is-active edit12-watcher.service) MainPID=$(systemctl --user show -p MainPID --value edit12-watcher.service)  本專案實例(cwd)=$(cnt_by_cwd hill_watcher.sh)"
