#!/bin/bash
# 乾淨 reset Edit11 的 dispatcher + hill_watcher 成「單一 systemd 管理實例」。
# 現行守護: systemd user services (edit11-dispatcher / edit11-watcher,
#           Restart=on-failure + enable-linger) — 已完全脫離 crontab race。
# 本腳本用於恢復: 殺光所有殘留/重複的 Edit11 daemon (含「相對路徑」啟動的孤兒),
#               再交回 systemd 單一管理。
...
systemctl --user stop edit11-dispatcher.service edit11-watcher.service 2>/dev/null
sleep 2
...
systemctl --user start edit11-dispatcher.service edit11-watcher.service 2>/dev/null
sleep 4

# 5) 報告 (用 cwd 判定本專案實例數, 應各 = 1; ★排除子shell: 父程序也是同腳本則不計)
cnt_by_cwd() { c=0; for p in $(pgrep -f "$1" 2>/dev/null); do
    last=$(tr '\0' '\n' </proc/"$p"/cmdline 2>/dev/null | tail -1)
    case "$last" in *"$1") : ;; *) continue ;; esac
    cwd=$(readlink /proc/"$p"/cwd 2>/dev/null)
    case "$cwd" in "$ROOT"|"$ROOT"/*) : ;; *) continue ;; esac
    ppid=$(awk '{print $4}' /proc/"$p"/stat 2>/dev/null)
    plast=$(tr '\0' '\n' </proc/"$ppid"/cmdline 2>/dev/null | tail -1)
    case "$plast" in *"$1") continue ;; esac   # 父也是同腳本 → 子shell, 不計
    c=$((c+1))
  done; echo "$c"; }
echo "=== daemon_reset done $(date '+%T') ==="
echo "dispatcher service=$(systemctl --user is-active edit11-dispatcher.service) MainPID=$(systemctl --user show -p MainPID --value edit11-dispatcher.service)  本專案實例(cwd)=$(cnt_by_cwd submit_dispatcher.sh)"
echo "watcher    service=$(systemctl --user is-active edit11-watcher.service) MainPID=$(systemctl --user show -p MainPID --value edit11-watcher.service)  本專案實例(cwd)=$(cnt_by_cwd hill_watcher.sh)"
