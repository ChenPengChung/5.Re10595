#!/bin/bash
# ============================================================================
# rearm_edit12_robot.sh — 登入時(或手動)重新武裝 Edit12 session-independent 機器人
# ----------------------------------------------------------------------------
...
#
# 安全:只碰 edit12-* unit + 全域 linger(benign,惠及所有 systemd --user 服務);
...
systemctl --user reset-failed edit12-watchdog.timer >/dev/null 2>&1 || true
systemctl --user enable --now edit12-watchdog.timer >/dev/null 2>&1 || true

# 3. grab timer:除非已成功搶到並自動解除,否則確保 enabled+started
if [ ! -f "$ROOT/restart/.grab_disarmed" ]; then
  systemctl --user reset-failed edit12-grab.timer >/dev/null 2>&1 || true
  systemctl --user enable --now edit12-grab.timer >/dev/null 2>&1 || true
fi
exit 0
