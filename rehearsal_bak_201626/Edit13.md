...
## 當前錨點（設定完成後填真值）
- **鎖定原則（單一真相）**：不在多處硬寫 account/partition/jp 期望值；一致性檢查**以「實際提交的 SLURM job」為唯一真相**
  （`scontrol show job $(cat restart/chain_jobid)` 的 `Account`/`Partition`/`NumNodes`），`jp_lock_selfcheck.sh` 已改為據此驗證
  各 config 檔是否與線上 job 一致。無線上 job（暫停中）時退回「各 config 檔互相一致」檢查，**不再用硬寫期望常數**。
- **註記 — Edit13 partition 切換清單（dev ⇄ 16gpus；2026-06-28 起鎖 16gpus）**：
  `115 | dev | 32jps`（06-28 前） ⇄ `115 | 16gpus | 32jps`（06-28 後）。**只動 partition + walltime；
  帳號 MST115169、jp 32 全程不變；`jp_lock_selfcheck.sh` 已去硬編碼、自動跟線上 job、不需改。**
  **完整 6 個定義檔**（去程 dev→16gpus；回程 16gpus→dev，值對調）：
  1. `chain_code_nchc/jobscript_chain.slurm.H200`：`#SBATCH --partition` `dev⇄16gpus` + `#SBATCH --time` `04:00:00⇄2-00:00:00`
  2. `restart/h200_partition`：`dev⇄16gpus`
  3. `chain_code_nchc/tools/select_combo_lib.sh`：`SC_PARTITIONS:-` `dev⇄16gpus`（`SC_ACCT`/`SC_VALID_JP` 不變）
  4. `chain_code_nchc/switch_partition.sh`：`LOCKED_PARTITION="dev"⇄"16gpus"`（FATAL 訊息用變數、自動跟）
  5. `chain_code_nchc/tools/partition_lib.sh`：`h200_known_partitions() { echo "dev"⇄"16gpus"; }`（walltime+cap map 已含 dev 與 16gpus，**不用補**）
  6. `chain_code_nchc/install_systemd.sh`：`printf 'dev'⇄'16gpus' > restart/h200_partition`（重裝時會覆寫此檔）
  - 不動：account（jobscript `--account` / `SC_ACCT` / `submit_dispatcher.sh` `ACCOUNT` 皆 `MST115169`）、jp（`variables.h` / `SC_VALID_JP` 皆 32）、`jp_lock_selfcheck.sh`（去硬編碼自動跟隨）。
  - 改完跑 `bash chain_code_nchc/tools/jp_lock_selfcheck.sh` 確認（與線上 job 一致；無 job 時看 config 自一致）。
