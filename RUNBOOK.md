...
> 目標:把 Edit11 (Re2800 / 513×257×257 / s0.95 / jp=8) 排進佇列,守望 daemon 一旦
...
| 本地 binary | `~/5.Re10595/Edit13_2800ITBLBM/a.out` | ✅ sm_70,0 warn/err,driver 550 支援 |
| chain wrapper | `~/5.Re10595/Edit13_2800ITBLBM/hill_local_chain.sh` | ✅ 冷/續 + 信號轉發 + exit碼契約 |
| 編譯腳本 | `~/5.Re10595/Edit13_2800ITBLBM/build_local.sh` | ✅ |
...
cfdq add --np 8 --model V100 --exclusive --chain --name edit11 -- bash hill_local_chain.sh
```
...
複查一次仍空 → 啟動 `hill_local_chain.sh` → 冷啟動讀預生網格 → 跑。
...
# 清重資料(測試後):
rm -f ~/5.Re10595/Edit13_2800ITBLBM/result/*.vtk ~/5.Re10595/Edit13_2800ITBLBM/result/*.bin
```
...
- **網格已預生**,節點冷啟不需 python。若改 `Re/NX/NY/NZ/jp/s` → 先重生網格再重編:
  ...
  bash build_local.sh                                # variables.h 改了就要重編
  ```
...
- **別碰原專案**:`~/5.Re10595/Edit11_Krank5600/` 有正在跑的 NCHC dispatcher/watcher;測試一律在 `~/5.Re10595/Edit13_2800ITBLBM/`。**絕不**把 cfdq 指到 `./run`(那是投 NCHC SLURM)。
