...
# accu_count 守門
if [ "${CK_ACCU:-0}" != "0" ] && [ "$FORCE_STATS_LOSS" -ne 1 ]; then
  die "accu_count=$CK_ACCU > 0: repartition_jp.py 不搬移湍流統計, 切 jp 會丟失已累積統計。
       若確定接受 (例如統計尚不重要), 重跑加 --force-stats-loss。"
fi
# RUNNING 守門
