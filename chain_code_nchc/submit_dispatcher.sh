...
presubdep_enabled() {
    [ "${CHAIN_PRESUBMIT_DEP:-}" = "1" ] && [ -f restart/presubmit_dep_enabled ]
}

presubdep_successor_alive() {
    local jid="$1"
    local state
    [[ "$jid" =~ ^[0-9]+$ ]] || return 1

    if command -v squeue >/dev/null 2>&1; then
        state="$(squeue -h -j "$jid" -o '%T' 2>/dev/null | head -1 | tr -d '[:space:]')"
        case "$state" in
    ...
    if command -v sacct >/dev/null 2>&1; then
        state="$(sacct -X -n -j "$jid" -o State 2>/dev/null | head -1 | awk '{print $1}' | tr -d '[:space:]')"
        case "$state" in
            PENDING|RUNNING|CONFIGURING|COMPLETING|RESIZING|SUSPENDED)
                return 0 ;;
        esac
...
while true; do
    ...
    if presubdep_enabled && [ -f restart/successor_jobid ]; then
        SUCC_JID="$(cat restart/successor_jobid 2>/dev/null | tr -d '[:space:]')"
        if presubdep_successor_alive "$SUCC_JID"; then
            log "[PRESUBDEP] successor already pending: job=$SUCC_JID; skip sbatch"
            sleep "$POLL_INTERVAL"
            continue
        fi
        log "[PRESUBDEP] successor marker not alive (${SUCC_JID:-empty}); falling through to normal sbatch"
    fi
