#!/bin/bash
# ============================================================================
# jp_lock_selfcheck.sh — Edit13_2800ITBLBM「32gpus@32jp 鎖」一致性深檢 (純唯讀)
# ----------------------------------------------------------------------------
# 本分支沿用既有 jp-lock 稽核模型；Edit13 的「鎖」是底下這幾個檔同時成立:
#   jp 鎖定 32 (variables.h #define jp 32 + select_combo_lib SC_VALID_JP 預設 32)
#   partition 鎖定 = 32gpus@jp32 (SC_PARTITIONS 預設 + jobscript header)
#   pin 若存在: restart/h200_partition = 32gpus
...
# 3. SC_PARTITIONS 預設 == 32gpus (硬鎖, 不含 8/16/64gpus)
...
# 4. jobscript header == 32gpus; restart/h200_partition 若存在也必須 == 32gpus
hdr=$(awk -F= '/^#SBATCH[[:space:]]+--partition=/{gsub(/^[ \t]+|[ \t]+$/,"",$2); print $2; exit}' "$JS_H200" 2>/dev/null)
if [ "${hdr:-}" = "$EXP_PIN" ]; then ok+=("jobscript partition=$hdr"); else drift+=("jobscript partition=${hdr:-?} (期望 $EXP_PIN)"); fi
...
if [ -n "${pin:-}" ]; then
    ...
    ok+=("h200_partition absent (jobscript header lock applies)")
fi
...
# ---- 結論 ----
if [ ${#drift[@]} -eq 0 ]; then
    [ "$QUIET" = 1 ] && echo "jp_lock OK: 32gpus@jp32 鎖一致 (${#ok[@]}/8 項通過)" \
                     || { echo "=== jp_lock_selfcheck: OK (32gpus@jp32 鎖一致) ==="; printf '  [✓] %s\n' "${ok[@]}"; }
    exit 0
fi
