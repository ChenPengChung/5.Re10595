...
#   * jp ≤ per-account GPU cap of the partition (live QOS 2026-06-05: 8gpus/16gpus/32gpus=32, dev=32, 64gpus=64)
...
#   由 QOS p_<partition> MaxTRESPerAccount 決定: dev=32, 8gpus=32, 16gpus=32, 32gpus=32, 64gpus=64。
#
...
sc_cap() {  # $1=partition -> per-account GPU cap (QOS MaxTRESPerAccount; live read, hardcoded fallback)
    ...
    #   p_dev=32, p_8gpus=32, p_16gpus=32, p_32gpus=32, p_64gpus=64.
    ...
    case "$part" in dev) echo 32 ;; 8gpus|16gpus|32gpus) echo 32 ;; 64gpus) echo 64 ;; *) echo 0 ;; esac
}
