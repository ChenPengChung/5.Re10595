...
#   dev=32, 8gpus=32, 16gpus=32, 32gpus=32, 64gpus=64。walltime = 各 partition MaxTime (live sinfo)。
h200_partition_walltime() {
    case "$1" in
        8gpus)   echo "2-00:00:00" ;;   # 2 day, QOS cap 32 GPU/acct
        16gpus)  echo "2-00:00:00" ;;   # 2 day, QOS cap 32 GPU/acct
        32gpus)  echo "1-00:00:00" ;;   # 1 day, QOS cap 32 GPU/acct (舊鎖定值)
        64gpus)  echo "1-00:00:00" ;;   # 1 day, QOS cap 64 GPU/acct (需 jp64 + 更細網格)
        dev)     echo "4:00:00"    ;;   # 4 hour, QOS cap 32 GPU/acct (jp32 適用)
        *)       echo "" ;;
    esac
}
