...
#   - restart/chain_jobid
#   - restart/successor_jobid                 (only when restart/presubmit_dep_enabled exists)
...
project_job_ids() {
    ...
    if [ -f restart/presubmit_dep_enabled ] && [ -f restart/successor_jobid ]; then
        id="$(cat restart/successor_jobid 2>/dev/null | tr -d '[:space:]' || true)"
        _is_numeric_id "$id" && echo "$id"
    fi
