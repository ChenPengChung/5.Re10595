---
description: Edit6 健康巡檢一輪 — watcher/dispatcher 存活 + partition×jp 切換機制稽核, 死亡則修碼+獨立推送, 沒問題則回報沒問題
argument-hint: (無參數; 連續監控請用 /loop /edit6-monitor)
---

你正在 `/home/s8313697/5.Re10595/Edit6_5600DNS`(GILBM periodic-hill Re5600 DNS)。
執行**一輪完整健康巡檢**,涵蓋下列 4 點。這條指令可在**任何 session** 重現相同行為;
要連續監控就讓使用者下 `/loop /edit6-monitor`(loop 會週期重跑本指令)。

## 判定方法(MUST — 唯一正確法)
- watcher / dispatcher 的**唯一真相來源 = systemd user service**:`edit6-watcher.service`、
  `edit6-dispatcher.service`。存活用 `systemctl --user is-active` + `show -p MainPID,NRestarts`。
- **殺/數 daemon 一律用 `/proc/PID/cwd` 判專案歸屬**(cwd 在 `Edit6_5600DNS` 才算本專案)—
...
## 巡檢 4 點
1. **存活**:
   - dispatcher:`systemctl --user is-active edit6-dispatcher.service`=active 且 cwd 實例=1。
   - watcher:同上(`edit6-watcher.service`)且 cwd 實例=1 且 `live/monitor_latest.png` < ~10 分;
