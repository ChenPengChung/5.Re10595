...
4 個 `#if` 開關只改公式那一行，不影響上下游邏輯：

| 切換點 | 1st order | 2nd order | 共用部分 |
|--------|-----------|-----------|---------|
| `BuildMrtProjectionTablesHost` | `Fu=Fv=Fw=0` | 完整 4-basis | `F0`, K 矩陣, 4 次 `ProjectForcingBasisHost` |
| `LegacyMrtCollisionHost` | `Fq=wq·f·3·cy` | 完整 Guo | moment transform, relaxation, forcing application |
| `VerifyMrtProjectionHost` | `F_direct=wq·3·cy` | 完整 split | basis error, projection error, moment check |
| `gilbm_bgk_collision_GTS` | `Fq=wq·F·3·cy` | 完整 Guo | feq, relaxation, force application |
| MRT GPU kernel | **無切換** | **無切換** | 一階時 `Fproj_u/v/w=0`，乘法結果自動為零 |

### 與參考專案 Duct 的對比

| 項目 | 本專案 (D3Q19) | Duct 參考專案 (D3Q27) |
|------|---------------|----------------------|
| 路徑 | `Edit5_Rebuild` | `/home/s8313697/D3Q27_PeriodicHill/Edit2_PeriodicHillDuct` |
