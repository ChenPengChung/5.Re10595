#!/usr/bin/env bash
# kill_zombie_watcher.sh — 清除「本專案(Edit12)所屬的殭屍 watcher」
# ============================================================================
...
#
# 守門:絕不碰 Edit6/Edit11 等別專案的 watcher(一律 /proc/PID/cwd 驗歸屬);只殺本機、
...
echo "=== Edit12 殭屍 watcher 清除 @ $(date '+%F %T') (本機=$MYHOST) ==="
...
for p in $(pgrep -f '[h]ill_watcher\.sh' 2>/dev/null || true); do
    [[ "$p" == "$$" || "$p" == "$PPID" ]] && continue
    # [硬化, 較 Edit11 版加固] 只認「程式=bash 且 最後參數=…hill_watcher.sh」的真 watcher 進程。
    #   pgrep -f 會誤命中 cmdline 內含字串 "hill_watcher.sh" 的非 watcher(如 codex exec 的 prompt、
    #   grep、編輯器), 它們 cwd 也可能==本專案 → 不排除就會被下面當殭屍誤殺。實測 codex exec 因
    #   prompt 提到 hill_watcher.sh 而被 pgrep -f 命中 → 故必擋。
    _argv0="$(tr '\0' '\n' < "/proc/$p/cmdline" 2>/dev/null | sed -n '1p')"
    _last="$(tr '\0' '\n' < "/proc/$p/cmdline" 2>/dev/null | grep -v '^$' | tail -1)"
    case "${_argv0##*/}::${_last}" in
        bash::*hill_watcher.sh) : ;;          # 真 watcher 進程(bash …/hill_watcher.sh)
        *) continue ;;                         # 非 watcher(codex/grep/編輯器等)→ 絕不碰
    esac
...
[[ "$killed_local" == 0 && "$kept_local" == 0 ]] && echo "  (本機無 Edit12 watcher 進程;合法者可能在別節點)"
...
if [[ -z "${seen_other[*]+set}" ]]; then   # set -u 安全:空關聯陣列不可用 ${#a[@]}(會 unbound abort)
    ...
    echo "          該節點時:readlink /proc/<pid>/cwd 確認是 Edit12 後 kill <pid>。"
fi
