...
// ║    rho/u/v/w  [GRID_SIZE] — macro read buffer                      ║
// ║    rho_d2/u2/v2/w2 [GRID_SIZE] — macro write buffer                ║
...
// └──────────────────────────────────────────────────────────────┘
#define     NX      449         // 展向格點
#define     NY      897         // 流向格點 (需 (NY-1)%jp==0; 896%32==0 → 896/32=28)
#define     NZ      449         // 法向格點
#define     jp      32         //  GPU 數量 (流向分割); NCHC 政策: 暫時鎖定 16gpus@32jp, 自由切換 {8gpus,16gpus,32gpus}@32jp (p_*gpus 每帳號 cap=32)
...
//   [本專案 NCHC 政策] NY=897 → 896 可被 8/16/32 整除 (896/8=112, 896/16=56, 896/32=28,
//     slab 皆 >=7) → jp∈{8,16,32} 皆合法; 暫時鎖定 jp=32 (partition 自由切換 {8gpus,16gpus,32gpus})。
#if ((NY-1) % jp != 0)
...
#define     FTT_STATS_START      20.0    // 統計量開始累積 (2026-06-03 提前: 50.0→20.0)
