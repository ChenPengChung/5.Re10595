...
// └──────────────────────────────────────────────────────────────┘
#define     NX      449         // 展向格點
#define     NY      897         // 流向格點 (需 (NY-1)%jp==0; 896%16==0 → 896/16=56)
#define     NZ      449         // 法向格點
#define     jp      16         //  GPU 數量 (流向分割); NCHC 政策: 暫時鎖定 16, 自由切換 {16,32,64}
...
//   [本專案 NCHC 政策] NY=897 → 896 可被 16/32/64 整除 (896/16=56, 896/32=28,
//     896/64=14, slab 皆 >=7) → jp∈{16,32,64} 皆合法; 暫時鎖定 jp=16。
#if ((NY-1) % jp != 0)
